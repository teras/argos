...
class TranslationSinglePassTest {
    ...
    fun `custom validation message translates exactly once`() {
        ...
        assertFalse(message.contains("[twice-validation]"), "Detected double translation in validation message: $message")
    }
    ...
    fun `help output translates developer strings exactly once`() {
        ...
        class HelpArgs(term: StringTerminal) : Arguments(appName = "double-check", appDescription = descKey, terminal = term) {
    ...
    fun `error messages translate only once even with translatable option names`() {
        ...
        val key = "Unknown option {option_token}{position}"
    ...
    fun `missing value messages translate only once with option token names`() {
        val key = "Missing value for {option_token}{position}"
    ...
    fun `domain error messages translate only once with domain names`() {
        val unknownDomainKey = "Unknown domain '{domain}'. Expected: {expected}{position}"
        ...
        assertTrue(message1.contains("[once-unknown-domain]"), "Expected translated unknown domain message, got: $message1")
        assertFalse(message1.contains("[twice-unknown-domain]"), "Detected double translation in unknown domain: $message1")
        ...
        assertTrue(message2.contains("[once-domain-required]"), "Expected translated domain required message, got: $message2")
        assertFalse(message2.contains("[twice-domain-required]"), "Detected double translation in domain required: $message2")
    }
    ...
    fun `arity validation messages translate only once`() {
        ...
        assertTrue(message.contains("[once-arity-validation]"), "Expected translated arity validation message, got: $message")
        assertFalse(message.contains("[twice-arity-validation]"), "Detected double translation in arity validation: $message")
    }
    ...
    fun `constraint template messages translate only once with argument names`() {
        ...
        assertTrue(message.contains("Option --option requires at least 2 occurrences"), "Expected raw constraint message, got: $message")
    }

    @Test
    fun `positional argument errors translate only once`() {
        val unexpectedKey = "Unexpected positional argument '{argument}'{position}"
    ...
    fun `option names matching message template placeholders translate only once`() {
        ...
        assertTrue(message.contains("[once-placeholder-match]"), "Expected single translation with placeholder names, got: $message")
        assertFalse(message.contains("[twice-placeholder-match]"), "Detected double translation with placeholder names: $message")
    }
