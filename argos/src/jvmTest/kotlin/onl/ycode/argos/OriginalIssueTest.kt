...
class OriginalIssueTest {
    ...
    class OriginalIssueArgs : Arguments() {
        ...
        val password1 by option().password(prompt = "Password 1", maxRetries = 1).required()
        val password2 by option().password(prompt = "Password 2", maxRetries = 1).required()
        ...
        val requiredCount by option("-v", "--verbose").bool().count().required()
    }
