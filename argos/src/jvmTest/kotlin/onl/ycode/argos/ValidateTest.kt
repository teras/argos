...
class ValidateTest {
    ...
    fun singleOptionValidation_templateVariables_replacedCorrectly() {
        class Args : Arguments(appName = "test", useANSITerminal = false) {
            val config by option("--config", "-c")
                .validate("File {value} not found for option {switches} ({name})") { it == "valid.conf" }
        }
        ...
        assertTrue(error.message!!.contains("File \"invalid.conf\" not found for option --config|-c (config)"))
    }
    ...
    @Test
    fun countOptionValidation_worksCorrectly() {
        class Args : Arguments(appName = "test", useANSITerminal = false) {
            val verbosity by option("-v", "--verbose").bool().count()
                .validate("Too verbose: {value} (max 3)") { it <= 3 }
        }

        // Valid: moderate verbosity
        val args1 = Args()
        args1.parseWithException(arrayOf("-vv"))
        assertEquals(2, args1.verbosity)

        // Invalid: too verbose
        val args2 = Args()
        val error = assertFailsWith<ParseError> {
            args2.parseWithException(arrayOf("-vvvv"))
        }
        assertTrue(error.message!!.contains("Too verbose: 4 (max 3)"))
    }
