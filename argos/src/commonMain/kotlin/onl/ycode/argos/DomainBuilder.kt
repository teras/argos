...
class DomainBuilder internal constructor(
    ...
    fun label(text: String) = apply {
        if (isFragment) throw ConfigError("Fragments cannot have labels")
    ...
    fun help(text: String) = apply {
        if (isFragment) throw ConfigError("Fragments cannot have help text")
    ...
    fun aliases(vararg names: String) = apply {
        if (isFragment) throw ConfigError("Fragments cannot have aliases")
     ...
     * @throws ConfigError if no domains provided, or if this is a fragment (fragments cannot inherit)
     */
    fun inherits(vararg domains: KProperty0<Boolean>) = apply {
        if (domains.isEmpty()) {
            throw ConfigError("inherits() requires at least 1 domain")
        }
        if (isFragment) {
            throw ConfigError("Fragments cannot inherit from other domains")
        }
    ...
    fun atLeast(ref: KProperty<*>, n: Int) = apply {
        if (n <= 0) throw ConfigError("atLeast requires n >= 1 (got $n) for '${ref.name}'")
    ...
    fun requireIfAnyPresent(target: KProperty<*>, vararg refs: KProperty<*>) = apply {
        ...
        if (t in names) throw ConfigError("Self is not allowed in requireIfAnyPresent refs for '$t'")
    ...
    fun requireIfAllPresent(target: KProperty<*>, vararg refs: KProperty<*>) = apply {
        val t = target.name
        val names = refs.map { it.name }.toSet()
        if (t in names) throw ConfigError("Self is not allowed in requireIfAllPresent refs for '$t'")
    ...
    fun requireIfAnyAbsent(target: KProperty<*>, vararg refs: KProperty<*>) = apply {
        val t = target.name
        val names = refs.map { it.name }.toSet()
        if (t in names) throw ConfigError("Self is not allowed in requireIfAnyAbsent refs for '$t'")
    ...
    fun requireIfAllAbsent(target: KProperty<*>, vararg refs: KProperty<*>) = apply {
        val t = target.name
        val names = refs.map { it.name }.toSet()
        if (t in names) throw ConfigError("Self is not allowed in requireIfAllAbsent refs for '$t'")
    ...
    fun exactlyOne(vararg refs: KProperty<*>) = apply {
        ...
        if (owners.size < 2) throw ConfigError("exactlyOne requires at least 2 refs")
    ...
    fun atMostOne(vararg refs: KProperty<*>) = apply {
        val owners = refs.map { it.name }.toSet()
        if (owners.size < 2) throw ConfigError("atMostOne requires at least 2 refs")
    ...
    fun atLeastOne(vararg refs: KProperty<*>) = apply {
        val owners = refs.map { it.name }.toSet()
        if (owners.size < 2) throw ConfigError("atLeastOne requires at least 2 refs")
    ...
    fun conflicts(vararg refs: KProperty<*>) = apply {
        val owners = refs.map { it.name }.toSet()
        if (owners.size < 2) throw ConfigError("conflicts requires at least 2 refs")
