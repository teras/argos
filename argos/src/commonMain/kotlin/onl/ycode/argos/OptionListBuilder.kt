...
class OptionListBuilder<T : Any> internal constructor(
     ...
     */
    fun required(): OptionListBuilder<T> = atLeast(1) as OptionListBuilder<T>
    ...
    fun arity(n: Int): ArityListOptionBuilder<T> {
        if (n <= 1) throw ConfigError("Arity must be at least 2 (got $n). Use regular list() for single values.")
