...
class OptionSetBuilder<T : Any> internal constructor(
     ...
     */
    fun required(): OptionSetBuilder<T> = atLeast(1) as OptionSetBuilder<T>
    ...
    fun arity(n: Int): AritySetOptionBuilder<T> {
        if (n <= 1) throw ConfigError("Arity must be at least 2 (got $n). Use regular set() for single values.")
