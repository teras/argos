...
Count options track how many times an option appears:

```kotlin
class MyApp : Arguments() {
    // Count occurrences
    val verbosity by option("-v", "--verbose").count()   // Int

    // Count with type conversion (less common)
    val debugLevel by option("-d", "--debug").count()
}
```
...
class BuildTool : Arguments() {
    ...
    val verbosity by option("-v", "--verbose").count()
...
// Use Count for cumulative flags
val verbosity by option("-v").count()            // -v, -vv, -vvv
```
